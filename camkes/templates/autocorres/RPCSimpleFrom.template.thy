...
lemma get_/*? m.name ?*/_/*? p.name ?*/_wp[wp_unsafe]:
  ...
  "\<lbrace>\<lambda>/*? state ?*/. globals_frame_intact /*? state ?*/ \<and>
        ...
        /*- endif -*/
        /*- for t in range(threads) -*/
          (\<forall>(a :: /*? word_type ?*//*? sizeof(p) * 8 ?*/ ptr)\<in>set (array_addrs (Ptr (symbol_table ''/*? m.name ?*/_/*? p.name ?*/_/*? t + 1 ?*/'')) seL4_MsgMaxLength). is_valid_w/*? sizeof(p) * 8 ?*/ /*? state ?*/ (ptr_coerce a)) \<and>
        /*- endfor -*/
        /*- for t in range(threads) -*/
(** TPP: accumulate = True *)
...
(** TPP: lock_indent = 6 + len("/*? state ?*/") + 2 *)
  ...
  /*- endfor -*/
     ...
     ipc_buffer_valid /*? state ?*/\<rbrace>!"
(** TPP: lock_indent = None *)
  apply (simp add:/*? me.from_interface.name ?*/_/*? m.name ?*/'_def)
