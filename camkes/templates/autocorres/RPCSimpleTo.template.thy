 ...
 * abstract seL4_SetMR.
 *)
autocorres [ts_rules = nondet, no_heap_abs = seL4_SetMR] "/*? thy ?*/.c"
...
lemma [simp]:"/*? i ?*/ \<ge> 0 \<and> /*? i ?*/ < seL4_MsgMaxLength \<Longrightarrow>
    globals_frame_intact (setMR /*? s ?*/ /*? i ?*/ /*? x ?*/) = globals_frame_intact /*? s ?*/"
  by (simp add:setMR_def globals_frame_intact_def)
lemma [simp]:"/*? i ?*/ \<ge> 0 \<and> /*? i ?*/ < seL4_MsgMaxLength \<Longrightarrow>
    ipc_buffer_valid (setMR /*? s ?*/ /*? i ?*/ /*? x ?*/) = ipc_buffer_valid /*? s ?*/"
  by (simp add:setMR_def ipc_buffer_valid_def)
lemma [simp]:"/*? i ?*/ \<ge> 0 \<and> /*? i ?*/ < seL4_MsgMaxLength \<Longrightarrow>
    tls_valid (setMR /*? s ?*/ /*? i ?*/ /*? x ?*/) = tls_valid /*? s ?*/"
  by (simp add:setMR_def tls_valid_def tls_ptr_def)
lemma [simp]:"/*? i ?*/ \<ge> 0 \<and> /*? i ?*/ < seL4_MsgMaxLength \<Longrightarrow>
    thread_index_C (tls (setMR /*? s ?*/ /*? i ?*/ /*? x ?*/)) = thread_index_C (tls /*? s ?*/)"
  by (simp add:setMR_def tls_def tls_ptr_def)
...
lemma /*? m.name ?*/_internal_nf:
  ...
  apply (simp add:/*? m.name ?*/_internal'_def)
  apply wp
   ...
   /*- endfor -*/
  apply (clarsimp|unfold inv_def, clarsimp|rule conjI, clarsimp|drule /*? me.to_interface.name ?*/_/*? m.name ?*/_inv)+
  done
(** TPP: condense = False *)

lemmas /*? m.name ?*/_internal_wp[wp_unsafe] =
  /*? m.name ?*/_internal_nf[THEN validNF_make_schematic_post, simplified]
